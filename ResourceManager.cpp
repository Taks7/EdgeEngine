		...
		for (int i = 0; i < files.size(); ++i)
		{
			if (files[i].find(".rg") != std::string::npos)
			{
				...
				if (App->fs->Load(metaFile.c_str(), &buffer) > 0)
				{
					...
					if (files[i].find(MODELS_EXTENSION) != std::string::npos) CreateResourceCreated(ResourceType::MODEL, uid, assets, dir + files[i]);
					else if (files[i].find(TEXTURES_EXTENSION) != std::string::npos) CreateResourceCreated(ResourceType::TEXTURE, uid, assets, dir + files[i]);
					else if (files[i].find(MESHES_EXTENSION) != std::string::npos) CreateResourceCreated(ResourceType::MESH, uid, assets, dir + files[i]);
...
Mesh* ResourceManager::IsMeshLoaded(std::string path)
{
	...
	{
		App->fs->GetFilenameWithExtension(p);
